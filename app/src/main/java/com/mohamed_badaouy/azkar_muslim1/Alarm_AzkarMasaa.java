...
public class Alarm_AzkarMasaa extends BroadcastReceiver {
	Date mDate;
	PowerManager pm;
	...
	public void onReceive(final Context context, Intent p2) {
		...
		if(RunAndCloseAlarmAzkar==true){
             if(mDate.getHours()==hourmasaa&&mDate.getMinutes()==minutemasaa){
				 /* if(show_WindowManager==false){}*/
				create_windo(context);
				myNotfication(context); 
				 sound.stop();
				  sound=MediaPlayer.create(context,R.raw.evening1);
				  sound.start();
				 sound.setOnCompletionListener(new MediaPlayer.OnCompletionListener(){

						 @Override
						 public void onCompletion(MediaPlayer p1) {
							 sound=MediaPlayer.create(context,R.raw.evening);
							 sound.start();
						 }
	...
	public void NextAlarm(Context context){
		Intent AzkarMasaa= new Intent(context,Alarm_AzkarMasaa.class);
        PendingIntent  AzkarMasaaPendingIntent = PendingIntent.getBroadcast(context, 0, AzkarMasaa, 0);
